...
inline float ToRad( float angle ) { return ( angle * 0.00277777778f * ( PI + PI ) ); }
...
public:
	...
	void Normalize() { const float l = 1.0f / Length(); x *= l;	y *= l; }
	vector2 Normalized() { const float l = 1.0f / Length(); return vector2( x*l, y*l );	}
	float Length() const { return sqrtf( x * x + y * y); }
	float SqrLength() const { return x * x + y * y;	}
	vector2 Cross( vector2& a_V ) const { return vector2(y * a_V.x - x * a_V.y,	x * a_V.y - y - a_V.x ); }
	float Dot( vector2& a_V ) const { return x * a_V.x + y * a_V.y; }
