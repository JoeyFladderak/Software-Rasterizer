...
public:
	...
	unsigned long Color::ToPixel()
	{
		unsigned int _r, _g, _b;
		float max_value = 255.f / ( MAX( 1, MAX( r, MAX( g, b ) ) ) );
		_r = (int)( r * max_value );
		_g = (int)( g * max_value );
		_b = (int)( b * max_value );
		return ( 0xff000000 + ( _r << 16 ) + ( _g << 8 ) + _b );
	}
	Color Color::FromPixel(unsigned long a_Color)
	{
		const float r = ((float)( ( a_Color & 0xFF0000 ) >> 16 )) / 255.f;
		const float g = ((float)( ( a_Color & 0x00FF00 ) >> 8 )) / 255.f;
		const float b = ((float)( a_Color & 0x0000FF )) / 255.f;
